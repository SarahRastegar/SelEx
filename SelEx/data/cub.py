...
class CustomCub2011(Dataset):
    ...
    def _load_metadata(self):
        ...
        image_attribute_labels = pd.read_csv(os.path.join(self.root, 'CUB_200_2011', 'attributes','image_attribute_labels.txt'),
                                       sep=' ' , names=['img_id', 'attribute','present'],usecols=[0, 1,2])

        data = images.merge(image_class_labels, on='img_id')
        attr = images.merge(image_attribute_labels, on='img_id')
        attr= attr.groupby('img_id').agg({'present':lambda x: list(x)})
        attr['present'] = attr['present'].apply(lambda x: np.asarray(x))
        #attr = attr.merge(train_test_split, on='img_id')
        #data= data.merge(train_test_split, on='img_id')
        self.data = data.merge(train_test_split, on='img_id')
        self.attr = attr#.merge(train_test_split, on='img_id')
        if self.train:
            self.data = self.data[self.data.is_training_img == 1]
            #self.attr = self.attr[self.attr.is_training_img==1]
        else:
            self.data = self.data[self.data.is_training_img == 0]
            #self.attr = self.attr[self.attr.is_training_img==0]
...
def subsample_dataset(dataset, idxs):
    ...
    #dataset.attr=dataset.attr[mask]
