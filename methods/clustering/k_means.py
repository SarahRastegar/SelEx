...
use_dataset = 'cub'#help='options: cifar10, cifar100, scars, aircraft, herbarium_19')


def test_kmeans_semi_sup(merge_test_loader, args, K=None, silence =True, is_code=False, unbalanced =False):
    ...
    for batch_idx, (feats, label, _, mask_lab_) in enumerate(tqdm(merge_test_loader, disable=silence)):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, default=use_dataset, help='options: cifar10, cifar100, scars, aircraft')
    ...
    if args.warmup_model_exp_id is not None:
        ...
        if not args.silent: print(f'Using features from experiment: {args.warmup_model_exp_id}')
    else:
        if not args.silent: print(f'Using pretrained {args.model_name} features...')

    if not args.silent: print(args.save_dir)

    # --------------------
    # DATASETS
    # --------------------
    if not args.silent: print('Building datasets...')
    ...
    torch.save(kmeans_feature.cluster_centers_, cluster_save_path)
